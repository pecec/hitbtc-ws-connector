...
from wss import WebSocketConnector
...
class HitBTCConnector(WebSocketConnector):
    ...
    def __init__(self, url=None, ssl_cert_file=None, raw=None,
                 stdout_only=False, silent=False ):
        """Initialize a HitBTCConnector instance."""
        url = url or 'wss://api.hitbtc.com/api/2/ws'
        super(HitBTCConnector, self).__init__( url, ssl_cert_file )
    ...
    async def put(self, item, block=False, timeout=None):
    ...
    async def recvResponse( self, _myCallback ):
        try:
            message = await self.conn.recv()
            #print('Received message from server: ' + str(message))
            await _myCallback( message )
            ...
            if 'jsonrpc' in decoded_message:
                if 'result' in decoded_message or 'error' in decoded_message:
                    ...
                    try:
                        ...
                        return False
                    await self._handle_stream(method, symbol, params)
        else:
            await self.put(message)
            
        return True

    async def onMessage( self, _myCallback ):
        continue_loop = True
        while continue_loop:
            continue_loop = await self.recvResponse( _myCallback )
#            try:
#                message = await self.conn.recv()
#                #print('Received message from server: ' + str(message))
#                await _myCallback( message )
...
#                await self.put(message)
    ...
    async def _handle_response(self, response):
        ...
        if 'result' in response:
            await self._handle_request_response(request, response)
        elif 'error' in response:
            await self._handle_error(request, response)

    async def _handle_request_response(self, request, response):
        """
        ...
        if method.startswith('subscribe'):
            ...
            self.echo('response: ' + formatted_msg)
        else:
        ...
        await self.put(('Response', 'Success', (request, response)))
    ...
    async def _handle_error(self, request, response):
        ...
        await self.put(('Response', 'Failure', (request, response)))

    async def _handle_stream(self, method, symbol, params):
        """Handle streamed data."""
        await self.put((method, symbol, params))

    async def send(self, method, custom_id=None, **params):
        """
    ...
    async def authenticate(self, key, secret, basic=False, custom_nonce=None):
