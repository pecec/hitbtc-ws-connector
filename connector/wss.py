...
class WebSocketConnector:
    ...
    def __init__(self, url, ssl_cert_file, timeout=None, q_maxsize=None, reconnect_interval=None,
                 log_level=None ):
    ...
    async def _connect( self ):
        ...
        #localhost_pem = pathlib.Path(__file__).with_name( 'hitbtc-com-chain.pem' )
        if not self.certFile is None:
            if not os.path.isfile( self.certFile ):
                raise ValueError( 'File doesn\'t exist: %s' % self.certFile )
            ssl_context.load_verify_locations( self.certFile )
        #self.conn.run_forever(sslopt=sslopt_ca_certs)
        #async with websockets.connect( uri, ssl=ssl_context ) as websocket:
        self.conn = await websockets.client.connect( self.url, ssl = ssl_context )
        if self.conn.open:
            self._is_connected = True
            print('Connection stablished. Client correcly connected')

        #return self.conn
        #asyncio.get_event_loop().run_forever( self.run() )
